...
"""
Training of baseline model with simple LSTMs for discourse relation sense classification (CoNLL16st).
"""
...
argp.add_argument('--rnn_num', type=int, default=1,
    help="number of stacked simple RNN layers (%(default)s)")
argp.add_argument('--rnn_type', default="lstm-fwd",
    choices=rnn_choices,
    help="recurrent layer for simple RNNs (%(default)s)")
argp.add_argument('--rnn_dim', type=int, default=12*20,
    help="size of simple RNNs (%(default)s)")
argp.add_argument('--rnn_act', default="tanh",
    choices=["relu", "tanh", "sigmoid", "softmax", "softplus", "linear"],
    help="activation function at output gates of simple RNNs (%(default)s)")
argp.add_argument('--rnn_dropout_W', type=float, default=0.0,
    help="dropout at input gates of simple RNNs (%(default)s)")
argp.add_argument('--rnn_dropout_U', type=float, default=0.0,
    help="dropout at recurrent gates of simple RNNs (%(default)s)")
argp.add_argument('--rnn_zoneout_h', type=float, default=0.0,
    help="zoneout at hidden units of simple RNNs (%(default)s)")
argp.add_argument('--rnn_zoneout_c', type=float, default=0.0,
    help="zoneout at cell units of simple RNNs (%(default)s)")
argp.add_argument('--rnn_shared', default="none",
    choices=["global", "none"],
    help="sharing of simple RNNs layers (%(default)s)")

argp.add_argument('--final_dim', type=int, default=0,
...
if args.rnn_zoneout_h > 0.0 or args.rnn_zoneout_c > 0.0:
