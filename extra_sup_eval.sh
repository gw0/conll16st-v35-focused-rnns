#!/bin/sh
# Re-evaluate additional scores, confusion matrices, and Brier score for our model with focused RNNs for discourse relation sense classifier (CoNLL16st).
#
...
for OUTPUT_DIR in models-*/*-$LANG-*/valid models-*/*-$LANG/valid; do ./conll16st_evaluation/extra_sup_eval.py "$VALID_DIR" "$OUTPUT_DIR" 2>&1 > $OUTPUT_DIR/extra_sup_eval.log; done
for OUTPUT_DIR in models-*/*-$LANG-*/test models-*/*-$LANG/test; do ./conll16st_evaluation/extra_sup_eval.py "$TEST_DIR" "$OUTPUT_DIR" 2>&1 > $OUTPUT_DIR/extra_sup_eval.log; done
for OUTPUT_DIR in models-*/*-$LANG-*/blind models-*/*-$LANG/blind; do ./conll16st_evaluation/extra_sup_eval.py "$BLIND_DIR" "$OUTPUT_DIR" 2>&1 > $OUTPUT_DIR/extra_sup_eval.log; done
