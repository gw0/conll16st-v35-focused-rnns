...
if [ -z "$TRAIN_DIR" -a -z "$MODEL_DIR" ]; then
  echo "Usage: ./v35/run.sh <model_dir> [en|zh] [extra_param]"
  exit -1
fi
...
if [ -d "$MODEL_DIR" ]; then
  ...
  ./v35/train.py "$MODEL_DIR" "$LANG" "$TRAIN_DIR" "$VALID_DIR" $EXTRA
fi
set -e -o pipefail
...
./v35/classify.py "$MODEL_DIR" "$LANG" "$VALID_DIR" "$OUTPUT_DIR/valid" $EXTRA
...
./v35/classify.py "$MODEL_DIR" "$LANG" "$TEST_DIR" "$OUTPUT_DIR/test" $EXTRA
...
./v35/classify.py "$MODEL_DIR" "$LANG" "$BLIND_DIR" "$OUTPUT_DIR/blind" $EXTRA
